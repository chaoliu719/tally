...
## Purpose
为破坏性写操作(当前是账户删除与分类删除)提供一个无状态、可防重放的 preview → apply 两步确认机制,避免误操作导致数据丢失,同时不需要服务端保存任何会话状态。未来新增的破坏性操作(例如批量修改交易)可以复用同一套确认规则。
...
- **WHEN** 携带的 `confirmation_token` 是为另一个操作或另一类资源签发的(例如为删除账户签发的令牌被用来确认删除分类)
- **THEN** 请求被拒绝,不执行操作
