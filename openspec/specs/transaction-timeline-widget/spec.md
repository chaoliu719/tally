...
## Purpose
让唯一用户在支持 MCP Apps 表面的宿主里,以一个内联的交互式面板从新到旧、按天分组、按页翻页地浏览某个账本的全部交易——直到最早一条,且浏览过程中的交易数据不进入 Agent 的对话上下文。同时约束 tally-mcp 托管此类 `ui://` widget 资源时的通用规则(自包含、无外部请求、宿主无此能力时降级为工具文本)。
...
时间线面板 SHALL 将交易按其发生日期(以用户本地时区解释 `time` 的 unix 秒)分组,日期段从最新到最早排列,每段内交易也按时间倒序。面板 SHALL 每屏固定展示至多 15 条交易,底部提供「页首 / 上一页 / 下一页 / 页尾」翻页控件与当前页码。当用户请求的页尚未取回时,面板 SHALL 通过 `callServerTool` 调用 `search_transactions`(`newest_first=true`,带上一次返回的 `cursor`)按需取回后续更早的数据;「页尾」SHALL 先取回全部剩余交易再跳到最后一页。已取回的数据在翻页时 SHALL 复用、不重复请求。翻页与取数请求 SHALL 不经过 Agent、不把交易数据写入对话上下文。
