...
用户 SHALL 能够通过 `get_financial_summary` 工具,指定 `ledger_id` 与一个可选的时间范围(`start_time`/`end_time`,unix 秒,语义与 `search_transactions` 一致),获得该账本内该范围内 `income`/`expense` 类型交易的总收入、总支出、净额(总收入减总支出),以及按分类、按来源拆分的收入/支出小计。金额按币种分组返回,不做任何汇率换算,不做跨账本汇总。
...
### Requirement: 按分类拆分收支
`get_financial_summary` 的返回结果 SHALL 包含按分类拆分的收入/支出小计:对指定账本统计范围内每个出现过 `income` 或 `expense` 交易的分类,给出该分类下的收入小计与支出小计(同样按币种分组),不包含其他账本的分类。
...
### Requirement: 按来源拆分收支
`get_financial_summary` 的返回结果 SHALL 包含按来源拆分的收入/支出小计:对指定账本统计范围内每个发生过 `income` 或 `expense` 交易的来源,给出该来源上的收入小计与支出小计,不包含其他账本的来源。
