...
用户 SHALL 能够通过 `create_transaction` 工具记录一笔收入或支出类型的交易,指定所属来源、分类、金额、币种与发生时间。币种是交易自身的字段(来源不再携带币种),按币种真实小数位数校验金额。分类可以是账本中任意已存在的分类,不要求处于某个特定层级。

#### Scenario: 提供有效信息记录交易
- **WHEN** 调用 `create_transaction`,指定的来源已存在、指定的分类已存在,金额、币种、发生时间均合法
- **THEN** 交易被记录,随后可以通过 `get_transaction` 按其 ID 查到

#### Scenario: 引用不存在的来源或分类
- **WHEN** 调用 `create_transaction`,指定的来源 ID 或分类 ID 在当前账本中不存在
...
#### Scenario: 缺少必填字段
- **WHEN** 调用 `create_transaction`,缺少来源、分类、金额、币种或发生时间中的任意一项
- **THEN** 请求被拒绝,不记录交易
...
### Requirement: 更新一笔交易
用户 SHALL 能够通过 `update_transaction` 工具,用交易 ID 替换一笔已存在交易的全部可变字段(`type`/`source_id`/`category_id`/`amount`/`currency`/`time`/`comment`)。这是完整字段替换语义,不支持只修改其中一部分字段;字段校验规则与 `create_transaction` 完全一致(income/expense 需要合法 `category_id` 且 `amount` 为正;`currency` 必须是系统支持的货币)。更新不需要二次确认,直接执行。

#### Scenario: 提供完整合法字段更新交易
- **WHEN** 调用 `update_transaction`,提供一个已存在交易的 ID,以及完整、合法的 `type`/`source_id`/`category_id`/`amount`/`currency`/`time`/`comment`(来源与分类均存在,金额/分类规则与指定的 `type` 相符,`currency` 受支持)
- **THEN** 该交易被替换为新值,随后 `get_transaction` 返回更新后的信息

#### Scenario: 缺少必填字段
- **WHEN** 调用 `update_transaction`,缺少 `id`、`type`、`source_id`、`amount`、`currency`、`time` 中的任意一项
- **THEN** 请求被拒绝,交易不发生变化

#### Scenario: 引用不存在的来源或分类
- **WHEN** 调用 `update_transaction`,指定的 `source_id` 或 `category_id` 在当前账本中不存在
...
### Requirement: 删除一笔交易
用户 SHALL 能够通过 `delete_transaction` 工具,以 preview → apply 两步确认(复用 `write-confirmation` 机制)删除一笔已存在的交易记录。与来源、分类删除不同,删除交易没有"是否被其他数据引用"这类前置门槛——任意已存在的交易都允许被删除。
...
### Requirement: 按条件搜索交易
用户 SHALL 能够通过 `search_transactions` 工具,按时间范围、来源、分类中的一个或多个条件筛选交易列表,并通过 `limit`/`cursor` 分批翻页取完结果。结果按 `time`、`id` 稳定排序;每次调用最多返回 `limit` 条(未提供时默认为 50,超过 200 的请求被拒绝),当还有更多结果时响应中带一个 `next_cursor`,把它原样传回下一次调用的 `cursor` 入参即可取得下一页;没有更多结果时不返回 `next_cursor`。
...
- **WHEN** 调用 `search_transactions` 提供的 `cursor` 无法解析,或是用不同的筛选条件(`source_id`/`category_id`/`start_time`/`end_time`)签发的
...
**Reason**: 来源不再有余额概念,"修正账户余额"这个需求本身不再成立。
**Migration**: 无替代——如果需要修正历史记录,直接用 `update_transaction`/`delete_transaction` 调整对应的 income/expense 交易即可。
