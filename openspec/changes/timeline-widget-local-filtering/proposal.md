...
- **筛选改为纯本地、即时生效**:过滤条不再有"应用"按钮 —— 修改任一输入(起始/结束日期、分类、来源)即刻重新过滤,不作废本地数据、不重新联网。起始/结束日期在 `time` 字符串的 `YYYY-MM-DD` 前缀上按字典序比较;分类/来源按 id 匹配;选中某分类时,用首屏返回的 `parent_id` 映射在本地展开其全部子孙分类一并匹配(等价于原 `include_descendants=true`)。筛选后翻页、分组、日合计全部基于本地过滤后的结果集。「重置」一键清空全部条件。
...
- widget 资源加载慢(~10s):给时间线 `ui://` 资源的 `resources/read` 结果加正的缓存 TTL(5 分钟)与 `public` 范围,取代 SDK 默认的"立即过期",宿主在 TTL 内复用缓存的 ~380KB HTML,不必每次打开面板/刷新页面都重拉。
- 修三个 widget bug:(a) 深色主题下进入/退出全屏变成浅色 —— 宿主上下文变化事件也会因展示模式切换触发且不带主题字段,面板此前把"无主题"当成浅色;改为只在事件带明确主题时更新配色。(b)「筛选」控件点击无反应 —— 过滤条此前靠 `[hidden]` 属性收起,而 `.filterbar { display: flex }` 在部分浏览器里覆盖了 UA 的 `[hidden]` 样式;改为用显式 `.open` class 控制展开/收起,并给「筛选」按钮加展开指示(caret)与活跃筛选圆点。(c) 进入全屏后「全屏」按钮不消失 —— 按钮可见性此前只在初始化时判一次;改为随宿主上下文的展示模式实时切换,全屏时隐藏、回到内联时再显示。
