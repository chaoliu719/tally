...
- 修两个 widget bug:(a) 深色主题下进入/退出全屏变成浅色 —— 宿主上下文变化事件也会因展示模式切换触发且不带主题字段,面板此前把"无主题"当成浅色;改为只在事件带明确主题时更新配色。(b)「筛选」控件点击无反应 —— 过滤条此前靠 `[hidden]` 属性收起,而 `.filterbar { display: flex }` 在部分浏览器里覆盖了 UA 的 `[hidden]` 样式;改为用显式 `.open` class 控制展开/收起,并给「筛选」按钮加展开指示(caret)与活跃筛选圆点。
